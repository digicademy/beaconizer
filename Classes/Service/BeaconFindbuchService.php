...
class BeaconFindbuchService implements SingletonInterface {
		...
		if ($sourceIdentifier) {
			...
			if ($response[0]) {
				...
				foreach ($response[3] as $key => $targetIdentifier) {
					$linkObject = $propertyMapper->convert(array('sourceIdentifier' => $sourceIdentifier, 'targetIdentifier' => $targetIdentifier), '\ADWLM\Beaconizer\Domain\Model\Links');
					$providerObject = $propertyMapper->convert(array('title' => $response[1][$key]), '\ADWLM\Beaconizer\Domain\Model\Providers');
					$linkObject->setProvider($providerObject);
					$result->attach($linkObject);
				}
					// possibly filter result by provider link patterns
				if (!empty($providers)) $result = $this->filterResultByProvider($result, $providers);
			}
		}
			// return results
		return $result;
	...
	private function sendRequest($sourceIdentifier) {
		return json_decode(GeneralUtility::getUrl('http://beacon.findbuch.de/seealso/pnd-aks?format=seealso&id=' . $sourceIdentifier, FALSE));
	}
	...
	private function reconstituteProviders($providers) {
		...
		foreach ($providers as $provider) {
			$providerObject = $propertyMapper->convert($provider, '\ADWLM\Beaconizer\Domain\Model\Providers');
	...
	private function filterResultByProvider(ObjectStorage $result, $providers) {
		$filteredResult = $this->objectManager->get('\\TYPO3\\CMS\\Extbase\\Persistence\\ObjectStorage');
		$reconstitutedProviders = $this->reconstituteProviders($providers);
		$linkPatterns = array();
		foreach ($reconstitutedProviders as $provider) {
			if ($provider->getLinkPattern()) $linkPatterns[$provider->getUid()] = $provider->getLinkPattern();
		}
