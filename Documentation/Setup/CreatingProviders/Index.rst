...
your TYPO3 backend. A provider in the sense of the Beaconizer is an entity that offers a
BEACON file on the web. Provider records are essential for *link harvesting* with the
scheduler as well as for the *SeeAlso plugin* if you would like to filter results.
...
the URL of the provider's BEACON file in the *FEED* field. A list of BEACON file URLs
...
the scheduler task starts harvesting the BEACON file of this provider. As you might recognize,
the *Metadata from file* tab holds all official BEACON metadata fields from the `specification
...
The *Target pattern* field is only relevant if you would like to filter links by specific providers
